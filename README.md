...
5. The file ```catalogsApp.sql``` contains a database dump with example data. After creating a database
   named ```catalogsApp```, just set the file as a source in mariadb CLI: ```source catalogsApp.sql```.
...
8. Run ```symfony check:requirements```. If it will show some issues, then fix it and run ```symfony server:start -d```.
9. **Enjoy :)**